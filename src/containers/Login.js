...
const Login = props => {
  ...
  return (
    <div className="login-container">
      {!phone ? (
        <UserPhoneForm onLoginClick={num => handleLoginClick(num)} />
      ) : !props.clientId ? (
        <ValidateCodeForm
...
const mapDispatchToProps = dispatch => ({
  sendCodetoPhone: phoneNumber =>
    dispatch(userActions.sendCodeToUser(phoneNumber)),
  validateCode: (phoneNUmber, code, onError) =>
    dispatch(userActions.validateCode(phoneNUmber, code, onError)),
  addClientCode: (code, onError) =>
    dispatch(userActions.registerUser(code, onError)),
