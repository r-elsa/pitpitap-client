...
const PlayNowPage = props => {
  ...
  const handleClick = i => {
    ...
    {
      i === 0 ? setPlay(true) : setPlay(false);
    }
    setActive(i);
  };
    ...
    <FlexContainer flexDirection="column" alignItems="center">
      {width < "800" && (
        ...
        >
          ...
          />
          <Text fontSize="35rem" fontFamily="Rounded1c_Medium" margin="auto">
            שחקו עכשיו
          </Text>
      ...
      >
        {items.map((item, index) => (
          <span key={index} onClick={() => handleClick(index)}>
            ...
            <Text
              fontSize={["15rem", "20rem"]}
              fontFamily={
                active == index ? "Rounded1c_Black" : "Rounded1c_Regular"
              }
              margin="auto"
            >
              {item.name}
